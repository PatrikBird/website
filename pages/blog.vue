        ...
        <div class="mt-12 max-w-lg mx-auto grid gap-4 lg:grid-cols-3 lg:max-w-none">
          <ContentList v-slot="{ list }" path="/blog/" :query="{where: { draft: { $ne:true } }, sort: { _file: -1 } }">
            <NuxtLink
              ...
              :key="article._path"
              :to="article._path"
              class="flex flex-col no-underline rounded-lg shadow hover:shadow-xl bg-zinc-50 dark:bg-zinc-800/50 overflow-hidden opacity-90 hover:opacity-100 dark:bg-primary-700 transition-opacity transition-shadow duration-100 ease-in-out"
            >
